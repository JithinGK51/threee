...
void main() {
  testWidgets('AnimatedLogo widget renders correctly', (
    WidgetTester tester,
  ) async {
    // Build the widget
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(
          body: AnimatedLogo(size: 200, backgroundColor: Color(0xFF414141)),
        ),
      ),
    );
    ...
    ); // Wait for bounce2 delay

    // Verify the widget is still there after animation
    expect(find.byType(AnimatedLogo), findsOneWidget);
  });

  testWidgets('AnimatedLogo respects size parameter', (
    WidgetTester tester,
  ) async {
